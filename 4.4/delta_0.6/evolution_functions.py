...
def create_100_mol_image(mol_list, file_name, fitness, logP, SAS, RingCount):
    ...
    for i,m in enumerate(mol_list):
        m.SetProp('_Name','%s %s %s %s' % (round(fitness[i], 3), round(logP[i], 3), round(SAS[i], 3), round(RingCount[i], 3))) 
    try:
